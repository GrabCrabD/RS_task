...
toDoApp::toDoApp(QWidget *parent) : QMainWindow(parent), ui(new Ui::toDoApp) {
  ...
  connect(ui->tableWidget, &QTableWidget::itemSelectionChanged, this,
          &toDoApp::handleRowSelectionChange);
  connect(&dialogForm, &editDialog::saveEditedTask, this,
          &toDoApp::getEditedTask);
...
void toDoApp::refreshTasks() {
  ...
  for (int row = 0; row < taskList.size(); ++row) {
    ...
    QTableWidgetItem *dateItem =
        new QTableWidgetItem(task.getDate().toString("dd.MM.yyyy"));
...
void toDoApp::loadTasks() {
  ...
  for (int i = 0; i < size; ++i) {
    ...
    task.setDate(
        QDate::fromString(settings.value("date").toString(), "dd.MM.yyyy"));
...
void toDoApp::applyFilter() {
  ...
  for (Task &task : taskList) {
    ...
    if (!filterDescription.isEmpty() &&
        !task.getDescription().contains(filterDescription)) {
      passFilter = false;
    }
